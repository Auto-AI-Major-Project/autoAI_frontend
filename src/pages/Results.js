...
export default function Results() {
  ...
  useEffect(() => {
    if (runId) {
      fetchRunDetails(runId);
    } else if (!modelResults && !cleaningResults) {
      fetchAllRuns();
    }
  }, [runId, modelResults, cleaningResults]);
  ...
  // Download functions for cleaning results
  const handleDownloadCleaningReport = async () => {
  try {
    const token = getAuthToken();
    const response = await fetch(
      `http://localhost:8000${cleaningResults.next_steps.download_report}?format=pdf`, // Changed from docx to pdf
      {
        headers: {
          'Authorization': `Bearer ${token}`
        }
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${cleaningResults.cleaned_filename}_REPORT.pdf`; // Changed extension
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(url);
    document.body.removeChild(a);
  ...
  // Loading state
  if (isLoading && !displayResults && !cleaningResults) {
    return (
      <div className="rounded-2xl p-6 bg-white border shadow-sm">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-2xl p-6 bg-white border shadow-sm">
                ...
                <table className="min-w-full bg-white border border-slate-200 rounded-lg">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">AUC</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">F1 Score</th>
          ...
          <div className="mt-8 flex gap-3 flex-wrap">
            <button
              onClick={handleDownloadModel}
              disabled={!displayResults?.run_id}
              className="px-4 py-2 bg-[#1A6B8E] text-white rounded-lg hover:bg-[#155a7a] transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
            >
              Download Best Model
            </button>
            <button 
      ...
      {/* No Results State */}
      {!displayResults && !cleaningResults && (
        <div className="rounded-2xl p-6 bg-white border shadow-sm">
          <div className="text-center py-8">
            <h2 className="text-2xl font-bold text-[#1A6B8E] mb-2">No Results Found</h2>
            <p className="text-slate-500 mb-6">
              No results available. Please run data cleaning or model training first.
            </p>
            <button 
              onClick={() => navigate('/')}
              className="px-4 py-2 bg-[#1A6B8E] text-white rounded-lg hover:bg-[#155a7a] transition-colors"
